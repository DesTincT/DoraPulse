...
test('webhooks/github: delivery idempotency (same X-GitHub-Delivery twice => second skipped)', async (t) => {
...
test('webhooks/github: domain dedup (same deployment_status, different deliveries => no double-count)', async (t) => {
  ...
  const r1 = await fastify.inject({
    method: 'POST',
    url: '/webhooks/github?projectKey=pak',
    headers: { ...baseHeaders, 'x-github-delivery': 'd-a' },
    payload,
  });
  assert.equal(r1.statusCode, 200);
  ...
  const r2 = await fastify.inject({
    method: 'POST',
    url: '/webhooks/github?projectKey=pak',
    headers: { ...baseHeaders, 'x-github-delivery': 'd-b' },
    payload,
  });
  assert.equal(r2.statusCode, 200);
